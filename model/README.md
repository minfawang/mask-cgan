...
Disclaimer: this directory is forked from [aitorzip/Pytorch-CycleGAN](https://github.com/aitorzip/PyTorch-CycleGAN): A clean and readable Pytorch implementation of CycleGAN (https://arxiv.org/abs/1703.10593)

## Prerequisites

Code is intended to work with ```Python 3.7```, it hasn't been tested with previous versions. Run the following commands to set up the virtual env and install all the dependencies.

```bash
pipenv shell
```

During training, set up tensorboard monitoring with:
...
- `mask6_mnist2svhn_h32_nres=3_simpled`. Trained for 12 epoches.
...
```bash
sh download_dataset.sh <dataset_name>
```
